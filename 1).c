...
void Euler_posicion(double gamma,double h,double *MSD){
    int i;
    *MSD=0;
    double aux;
    for(i=0;i<N;i++){
...
int main(){
    ...
    for(j=0;j<3;j++){
        ...
        FILE*f;
        char filename[40];
        sprintf(filename,"MSD_tau%d.txt",tau);
        f=fopen(filename,"w");

        FILE*f1;
        char filename1[40];
        sprintf(filename1,"Trayectoria_tau%d.txt",tau);
        ...
        for(i=0;i<Nt;i++){
            MSD=0;
            Euler_ruido(tau,h,D);
            Euler_posicion(gamma,h,&MSD);
            delta_2=delta2(tau,i*h,D);
            MSD=MSD/(double)N;
            if(i%100==0){
                fprintf(f,"%lf       %lf     %lf\n",i*h,MSD,delta_2);
            }
